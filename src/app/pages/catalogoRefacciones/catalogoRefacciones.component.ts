...
export class CatalogoRefaccionesComponent
  implements OnInit, OnDestroy, AfterViewInit
{
  columnDefinitions = [
    { def: 'select', type: 'check', visible: true },
  ...
  agregarNuevaRefaccion() {
    const dialogRef = this.dialog.open(DialogCrearRefaccionComponent, {
      width: '450px',
    });
  ...
  editarRefaccion(refaccion: refaccionModel) {
    ...
    this.refaccionesService.getRefacciones('').subscribe({
      ...
      error: (err) => {
        ...
        this.showNotification(
          'snackbar-danger',
          'Error al cargar las refacciones.',
          'bottom',
          'center'
        );
      },
