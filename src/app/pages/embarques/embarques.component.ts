...
import { DialogAprobarEmbarqueComponent } from './dialogs/dialog-aprobar-embarque/dialog-aprobar-embarque.component';
...
export class EmbarquesComponent {
  columnDefinitions = [
    { def: 'id_embarque', label: '#', type: 'idTabla', visible: true },
    //{ def: 'id_proveedor', label: 'Nombre', type: 'text', visible: true },
  ...
  dataSource = new MatTableDataSource<embarqueModel>([]);
  selection = new SelectionModel<embarqueModel>(true, []);
  ...
  openDialog(action: 'add' | 'edit', data?: embarqueModel) {
  ...
  private updateRecord(updatedRecord: embarqueModel) {
    const index = this.dataSource.data.findIndex(
      (record) => record.id_embarque === updatedRecord.id_embarque
    );
    if (index !== -1) {
  ...
  loadData() {
    ...
    this.EmbarqueService.getAllEmbarques("").subscribe({
      next: (data) => {
        ...
        this.dataSource.filterPredicate = (data: embarqueModel, filter: string) => {
  ...
  onContextMenu(event: MouseEvent, item: embarqueModel) {
